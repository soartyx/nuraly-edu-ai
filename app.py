...
class MediaGenerator:
    ...
    async def fetch_video(self,keyword:str,path:Path)->Path:
        ...
        if not vids:raise RuntimeError(f"No video: {keyword}")
    ...
    async def generate_step(self,step:dict,idx:int,tmp:Path)->dict:
        ...
        audio,video=await asyncio.gather(self.tts(step["n"],ap),self.fetch_video(step["k"],vp))
        return{"title":step["t"],"audio":audio,"video":video,"formula":step.get("f")}
...
class VideoAssembler:
    def assemble(self,segments:list[dict],out:Path)->Path:
        ...
        for seg in segments:
            ...
            dur=audio.duration
            vid=(VideoFileClip(str(seg["video"])).without_audio()
                 .subclip(0,dur).loop(duration=dur).resize(height=720))
            layers=[vid]
            try:
                tc=TextClip(seg["title"],fontsize=40,color="white",font="Arial-Bold",
                            method="caption",size=(vid.w,None),
                            stroke_color="black",stroke_width=2)\
                    .set_position(("center",0.80),relative=True).set_duration(dur)
                layers.append(tc)
            except Exception as e:print(f"[warn] title: {e}")
            if seg.get("formula"):
                try:
                    fc=TextClip(seg["formula"],fontsize=36,color="yellow",font="Arial-Bold",
                                method="caption",size=(vid.w,None),
                                stroke_color="black",stroke_width=2)\
                        .set_position(("center",0.55),relative=True).set_duration(dur)
                    layers.append(fc)
                except Exception as e:print(f"[warn] formula: {e}")
            clips.append(CompositeVideoClip(layers).set_audio(audio))
        final=concatenate_videoclips(clips,method="compose")
        final.write_videofile(str(out),fps=24,codec="libx264",
                              audio_codec="aac",temp_audiofile="temp_a.m4a",remove_temp=True)
        return out
...
async def pipeline(text:str)->tuple[Path,dict]:
    ...
    segs=await asyncio.gather(
        *[MediaGenerator().generate_step(s,i,tmp) for i,s in enumerate(data["video_steps"])]
    )
...
st.markdown("<p style='text-align:center;color:#aaa'>Введи тему — получи видеоурок с квизом</p>",
            unsafe_allow_html=True)
st.divider()
...
topic=st.text_area("📚 Тема урока",placeholder="Например: Второй закон Ньютона...",height=120)
...
if st.session_state.video and Path(st.session_state.video).exists():
    ...
    for i,q in enumerate(quiz):
        with st.container():
            ...
            chosen=st.radio(f"q{i}",opts,index=None,label_visibility="collapsed",
                            key=f"radio_{i}")
            ...
            if st.button("Проверить",key=f"check_{i}"):
                if not chosen:
                    ...
                    correct_idx=q["a"]
                    st.session_state.checked[i]=(user_idx==correct_idx,correct_idx,q["o"])
            ...
            if i in st.session_state.checked:
                ...
                if ok:
                    ...
                    st.error(f"❌ Ошибка. Правильный ответ: **{LETTERS[ci]}) {options[ci]}**")
