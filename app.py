...
def render_formula_image(formula:str,w:int=1280,h:int=720)->PIL.Image.Image|None:
    try:
        ...
        ax.text(0.5,0.5,f"${clean}$",fontsize=80,color="white",
    ...
    except Exception as e:print(f"[formula] {e}");return None

def render_scheme_image(desc:str,title:str,w:int=1280,h:int=720)->PIL.Image.Image|None:
    try:
        import matplotlib;matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        for i,(x,part) in enumerate(zip(xs,parts)):
            ...
            ax.add_patch(FancyBboxPatch((x-1.05,2.3),2.1,1.4,
                         boxstyle="round,pad=0.12",facecolor=c+"33",
        ...
        return _save_fig(fig,w,h)
    except Exception as e:print(f"[scheme] {e}");return None

def render_code_image(code:str,title:str,w:int=1280,h:int=720)->PIL.Image.Image|None:
    try:
        import matplotlib;matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        from matplotlib.patches import FancyBboxPatch
        ...
        ax.text(5,5.15,title[:50],ha="center",va="center",color="#aaa",
                fontsize=11,fontweight="bold")
        kw={"def","class","return","import","for","if","else","elif","while",
            "in","and","or","not","True","False","None","from","with","lambda",
            "try","except","finally","yield","async","await"}
        lines=code.strip().split("\n")[:14]
        for li,line in enumerate(lines):
            y=4.6-li*0.32;words=line.split(" ");x_cur=0.5
            for word in words:
                col=("#F72585" if word.rstrip("(:") in kw
                     else "#FFD60A" if word.startswith("#")
                     else "#7BF1A8" if (word.startswith('"') or word.startswith("'"))
                     else "#e2e8f0")
                ax.text(x_cur,y,word+" ",ha="left",va="center",
                        color=col,fontsize=9.5,fontfamily="monospace")
                x_cur+=len(word)*0.095+0.095
                if x_cur>9.3:break
        return _save_fig(fig,w,h)
    except Exception as e:print(f"[code] {e}");return None

def render_chart_image(title:str,w:int=1280,h:int=720)->PIL.Image.Image|None:
    try:
        import matplotlib;matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        fig,ax=plt.subplots(figsize=(w/100,h/100),dpi=100,facecolor="none")
        ...
        ax.text(0.5,0.92,title[:55],transform=ax.transAxes,
                ha="center",color="#48CAE4",fontsize=18,fontweight="bold")
        return _save_fig(fig,w,h)
...
class MediaGenerator:
    ...
    FALLBACK=["abstract motion graphics 4k","dark particles flow loop",
              "minimalist geometry wave","deep space nebula loop","data network flow"]
    ...
    async def _try_fetch(self,query:str,path:Path)->Path|None:
        try:
            async with httpx.AsyncClient(timeout=30) as h:
                r=await h.get(self.PEXELS_URL,headers={"Authorization":PK},
                              params={"query":query,"per_page":5,"orientation":"landscape"})
            r.raise_for_status()
            vids=r.json().get("videos",[])
            if not vids:return None
            best=max(vids[0]["video_files"],key=lambda f:f.get("width",0)*f.get("height",0))
            async with httpx.AsyncClient(timeout=90,follow_redirects=True) as h:
                resp=await h.get(best["link"])
            path.write_bytes(resp.content);return path
...
class VideoAssembler:
    def assemble(self,segments:list[dict],out:Path)->Path:
        from moviepy.editor import (VideoFileClip,AudioFileClip,
            ...
            CompositeVideoClip,TextClip,ImageClip,AudioArrayClip)
        clips=[]
        for seg in segments:
            ...
            if mid_img:
                try:
                    ...
                    ic=(ImageClip(arr,ismask=False)
                        .set_duration(dur).set_position("center").set_opacity(0.9))
                    layers.append(ic)
                except Exception as e:print(f"[warn] viz: {e}")

            try:
                tc=(TextClip(seg["title"],fontsize=44,color="white",font="Arial-Bold",
                             method="caption",size=(int(W*0.88),None),
                             stroke_color="black",stroke_width=2)
                    .set_position(("center",0.84),relative=True).set_duration(dur))
                layers.append(tc)
            except Exception as e:print(f"[warn] title: {e}")
            ...
            if dur>speech_dur:
                ...
                silence=AudioArrayClip(
                    np.zeros((int(sr*(dur-speech_dur)),2),dtype=np.float32),fps=sr)
        ...
        final.write_videofile(str(out),fps=24,codec="libx264",
                              audio_codec="aac",temp_audiofile="temp_a.m4a",
                              remove_temp=True,threads=4,logger=None)
        return out
...
async def pipeline(text:str,progress)->tuple[Path,dict]:
    ...
    try:
        ...
        for b in range(0,n,batch_size):
            ...
            pct=15+int(60*(b/n))
            progress.progress(pct,f"⬇️ Загружаю шаги {b+1}–{min(b+batch_size,n)} / {n}...")
...
st.markdown("<div class='hero-sub'>Введи тему — получи глубокий видеоурок (20+ слайдов) с квизом</div>",
            unsafe_allow_html=True)
st.divider()
...
topic=st.text_area("Тема",placeholder="Например: Алгоритмы сортировки в Python, Законы термодинамики...",
...
if st.session_state.video and Path(st.session_state.video).exists():
    ...
    if data.get("sum"):
        st.markdown(f"<div class='card'>💡 <b>Итог:</b> {data['sum']}</div>",
                    unsafe_allow_html=True)

    # ── Квиз (6 вопросов) ────────────────────────────────────────────
    st.divider()
    st.markdown("<div style='font-family:Syne,sans-serif;font-size:1.5rem;"
    ...
    for i,q in enumerate(quiz):
        st.markdown("<div class='card'>",unsafe_allow_html=True)
        st.markdown(f"<span class='pill'>Вопрос {i+1} / {len(quiz)}</span>",
                    unsafe_allow_html=True)
        st.markdown(f"**{q['q']}**")
        correct_idx=q["a"]
        options=q["o"]
        ...
        if not already:
            ...
            for idx,option in enumerate(options):
                with cols[idx]:
                    if st.button(f"{LETTERS[idx]}) {option}",
                                 key=f"q_{i}_{idx}",use_container_width=True):
            ...
            for idx,option in enumerate(opts_s):
                with cols[idx]:
                    if idx==ci:st.success(f"{LETTERS[idx]}) {option}")
                    else:
                        st.markdown(f"<div style='padding:.45rem .6rem;border-radius:8px;"
        ...
        if i in st.session_state.checked:
            ...
            if ok:st.success("✅ Красавчик, правильно!")
            else:st.error(f"❌ Эх, мимо! Правильный: **{LETTERS[ci]}) {options[ci]}**")
    ...
    if len(st.session_state.checked)==len(quiz) and quiz:
        ...
        if pct==100:
            st.success("🏆 Идеально! Ты полностью освоил тему!");st.balloons()
        elif pct>=70:st.info("👍 Отличный результат! Материал усвоен хорошо.")
        elif pct>=50:st.warning("📖 Неплохо, но стоит повторить слабые места.")
        else:st.error("🔁 Рекомендую пересмотреть урок целиком.")
        st.markdown("</div>",unsafe_allow_html=True)